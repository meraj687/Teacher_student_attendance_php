...
<body>
 <div class="panel panel-default container">
  ...
  <div class="panel-body">
   ...
   <?php echo date('Y/m/d') ?>
   <a href="add1.php" class="btn btn-primary pull-right" style="float:right">Add</a>
...
?>
 ...
 <tr>
  ...
  <td>
   Present<input required type="radio" name="attendance1[<?php echo $show['reg_id']  ?>]" value="Present">
   Absent<input required type="radio" name="attendance1[<?php echo $show['reg_id']  ?>]" value="Absent" type="text">
  </td>
 </tr>
<?php } ?>
  ...
  if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    if(!$b)
    {
      foreach($att as $key=>$value){
        if($value=='Present'){
          $query="insert into attendance1(value,reg_id,date) values('Present','$key','$date')";
          $insertResult=$link->query($query);
        }
        else
        {
        $query="insert into attendance1(value,reg_id,date) values('Absent','$key','$date')";
          $insertResult=$link->query($query);
        }
      }
      if($insertResult)
